...
impl UpgradeManager {
    pub fn generate_upgraded_content(
        ...
        let lines = original_content.lines().collect::<Vec<_>>();
        ...
        for line in lines {
            ...
            let should_upgrade = packages
                ...
                .find(|p| {
                    ...
                    p.name.chars().all(|c| line_lower.contains(c))
                })
                .map(|p| {
                    if only_selected {
    ...
    pub fn write_lock_file<P: AsRef<Path>>(
        ...
        lines.push("# This file is automatically generated by PyElevate".to_string());
        lines.push(format!(
            "# Generated at {}",
...
fn generate_requirement_line(pkg: &Package, new_version: &str) -> String {
    ...
    match &pkg.constraint {
        VersionConstraint::Pinned(_) => format!("{}{}=={}", pkg.name, extras_str, new_version),
        VersionConstraint::GreaterEqual(old) => {
            format!("{}{}>={}  # Updated from {}", pkg.name, extras_str, new_version, old)
        }
        VersionConstraint::Compatible(_) => {
            format!("{}{}~={}  # Updated", pkg.name, extras_str, new_version)
        }
        VersionConstraint::Range(_, _) => {
            format!("{}{}=={}  # Updated", pkg.name, extras_str, new_version)
        }
        VersionConstraint::Less(_) => format!("{}{}=={}", pkg.name, extras_str, new_version),
        VersionConstraint::Unspecified => format!("{}{}=={}", pkg.name, extras_str, new_version),
    }
}
