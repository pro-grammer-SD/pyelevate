...
#[command(about = "Modern Python requirements.txt dependency upgrader with interactive TUI", long_about = None)]
...
async fn main() -> Result<()> {
    ...
    info!("Starting PyElevate with requirements file: {}", requirements_path);
...
async fn check_command(requirements_path: &str) -> Result<()> {
    ...
    println!("PyElevate - Dependency Check Report");
    println!("====================================\n");
    println!("Total packages: {}", app.stats.total);
    println!("Patch updates available: {}", app.stats.patch_available);
    println!("Minor updates available: {}", app.stats.minor_available);
    println!("Major updates available: {}", app.stats.major_available);
    println!("Up to date: {}", app.stats.up_to_date);
    println!("Errors: {}", app.stats.errors);

    println!("\n{:<30} {:<15} {:<15} {:<15}", "Package", "Current", "Latest", "Status");
    println!("{}", "-".repeat(75));
    ...
    for pkg in &app.packages {
        ...
        println!(
            "{:<30} {:<15} {:<15} {:<15}",
            pkg.name, pkg.current_version, latest, status
        );
    }
...
async fn upgrade_command(requirements_path: &str, dry_run: bool, lock: bool) -> Result<()> {
    ...
    let pypi_client = crate::pypi::PyPIClient::new();

    println!("PyElevate - Dependency Upgrade");
    println!("===============================\n");
    ...
    println!("\nAvailable upgrades: {}", upgradable.len());
...
async fn run_app(terminal: &mut Terminal<CrosstermBackend<io::Stdout>>, app: &mut App) -> Result<()> {
    ...
    app.loading_message = "Fetching package info from PyPI...".to_string();
    ...
    match (key.code, key.modifiers) {
        ...
        (KeyCode::Char('s') | KeyCode::Char('S'), _) => {
            app.sort_by = match app.sort_by {
                ...
                app::SortBy::Latest => app::SortBy::Name,
            };
            app.apply_sort();
        }
        (KeyCode::Enter, _) => {
            if app.count_selected() > 0 {
                ...
                app.set_error("Please select packages to upgrade (Space to select)".to_string());
            }
        }
        (KeyCode::Char(c), _) => {
